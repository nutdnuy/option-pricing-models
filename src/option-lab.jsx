...
import {bachelierCall,black76Call,bsmCall} from './math.mjs';
...
export function OptionModelLab(){
  ...
  const values=useMemo(()=>Array.from({length:41},(_,i)=>{
    const k=Math.max(1,underlying*0.6+i*underlying*0.02);
    return [k,price(model,underlying,k,rate,volPct,normalVol,time)];
  ...
  return <section className="lab" aria-label="ห้องทดลองเปรียบเทียบแบบจำลองราคาออปชัน">
    ...
    <div className="controls two"><div>
      ...
      <select id="option-model" value={model} onChange={e=>setModel(e.target.value)}><option value="bsm">Black–Scholes–Merton</option><option value="black76">Black-76</option><option value="bachelier">Bachelier</option></select>
      <Range label={modelCopy[model].input} value={underlying} onChange={setUnderlying} min={model==='bachelier'?-20:20} max={180} step={1}/>
      <Range label="Strike" value={strike} onChange={setStrike} min={1} max={180} step={1}/>
      ...
      <div className="results"><div><span>European Call</span><strong>{format(value,3)}</strong><p>หน่วยราคา</p></div><div><span>Time value</span><strong>{format(Math.max(0,value-intrinsic),3)}</strong><p>เหนือ discounted intrinsic</p></div></div>
    </div></div>
