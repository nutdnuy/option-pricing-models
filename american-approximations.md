...
ให้ $\varepsilon(S,\tau) = V^{A}(S,\tau) - V^{E}(S,\tau)$ เป็นส่วนเพิ่มของการใช้สิทธิล่วงหน้า โดย $\tau = T-t$ และให้ $b$ เป็นต้นทุนการถือครอง เนื่องจากทั้ง $V^{A}$ และ $V^{E}$ สอดคล้องกับ PDE แบล็ก-โชลส์เวอร์ชันทั่วไป $$\begin{equation}
\label{eq:baw-pde}
